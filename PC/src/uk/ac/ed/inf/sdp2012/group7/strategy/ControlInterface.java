...
public class ControlInterface implements Observer {
	...
	public void implementAStar(Plan plan) {
		synchronized (this){
			List<Point> path = (List<Point>) plan.getPath().clone();
			...
			while (!path.isEmpty()) {
				...
				if(path.size() > 2) {
					...
					secondPoint = path.get(index);

					if (plan.getOurRobotPositionVisual().distance(p1) > plan.getNodeInPixels()*Math.sqrt(2)/2) {
						firstPoint = plan.getOurRobotPositionVisual();
					}
